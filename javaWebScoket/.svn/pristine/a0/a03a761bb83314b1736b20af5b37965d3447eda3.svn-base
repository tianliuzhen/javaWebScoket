...
public class WebSocketTest {
     ...
     */
    @OnOpen
    public void onOpen(@PathParam(value = "userno") String param, Session session, EndpointConfig config) throws UnsupportedEncodingException {
        System.out.println(param);
        userno = param;//接收到发送消息的人员编号
        this.session = session;
        webSocketSet.put(param, this);//加入map中
        addOnlineCount();           //在线数加1
        System.out.println( URLDecoder.decode(param,"utf-8")+"用户新加入"+"有新连接加入！当前在线人数为" + getOnlineCount());
       ;
    }
